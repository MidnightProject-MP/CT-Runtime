...
* **Migrations:** Applied via direct migrator `npm run migrate -- --database-url <direct>` (`celestan_migrator` direct `npg_...`, *not* `celestan_runtime` pooled) → `{"status":"migrated","applied":[1,2,3,4,5,6,7,8]}` including execution federation, authenticated GAS transport, required handoff revision, and non-null handoff target. Verified migration checksums are canonical. Normal wake never runs migrations.
