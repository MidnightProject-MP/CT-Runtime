...
(function () {
  ...
  function hex(bytesValue) { return bytesValue.map(function (b) { return ('0' + (b < 0 ? b + 256 : b).toString(16)).slice(-2); }).join(''); }
  function digest(value) { return hex(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, value, Utilities.Charset.UTF_8)); }
  function canonicalName(name) { return name === 'appsscript.json' || name === 'appsscript' ? 'appsscript' : name.replace(/\.(?:js|mjs|html)$/i, ''); }
  function normalizeFiles(files) {
    ...
    files.forEach(function (file) {
      ...
      if (name === 'appsscript') { if (file.type !== 'JSON') throw new Error('deploy-invalid-manifest-type'); manifest++; }
      else if (file.type === 'JSON') throw new Error('deploy-unexpected-json-file');
  ...
  function bundleHash(files) { return digest(JSON.stringify(normalizeFiles(files))); }
