var CT_GAS_TRIGGER = (function () {
  ...
  function legacyRetirement(operation) {
    var mode = PropertiesService.getScriptProperties().getProperty('CT_AUTONOMY_MODE');
    return mode === 'vnext' ? { status: 'RETIRED_VNEXT', operation: operation, autonomy_mode: 'vnext' } : null;
  }
  ...
  return { registry:registry, ensure:ensure, schedule:schedule, due:due, retire:retire, legacyRetirement:legacyRetirement };
}());
...
function requestNextWake(wake) { var retired=CT_GAS_TRIGGER.legacyRetirement('requestNextWake'); if (retired) return retired; return CT_GAS_TRIGGER.schedule(wake); }
