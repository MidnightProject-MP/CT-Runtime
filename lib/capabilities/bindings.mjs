...
const CAP_ENV_MAP = {
  ...
  disposable_compute: 'CELESTAN_DISPOSABLE_COMPUTE',
  scheduler: 'CELESTAN_SCHEDULER', execution_federation: 'CELESTAN_EXECUTION_FEDERATION'
  ,agent_executor: 'CELESTAN_AGENT_EXECUTOR', workspace: 'CELESTAN_WORKSPACE', test_executor: 'CELESTAN_TEST_EXECUTOR', model_provider: 'CELESTAN_MODEL_PROVIDER'
};
...
const DEFAULTS = Object.freeze({
  production: Object.freeze({
    ...
    disposable_compute: 'northflank_sandbox',
    scheduler: 'northflank',
     execution_federation: 'postgres',
    project_system: 'github_issues', agent_executor: 'missing', workspace: 'missing', test_executor: 'missing', model_provider: 'missing'
  }),
  filesystem: Object.freeze({
    ...
    code_repository: 'github',
    knowledge_publishing: 'git',
    disposable_compute: 'filesystem',
    scheduler: 'cron',
     execution_federation: 'postgres',
    project_system: 'github_issues', agent_executor: 'missing', workspace: 'missing', test_executor: 'missing', model_provider: 'missing'
  }),
  gas: Object.freeze({
    durable_state: 'gas', evidence_store: 'gas', scheduler: 'gas', agent_executor: 'gas', workspace: 'gas', test_executor: 'gas', model_provider: 'gas',
    code_repository: 'github', knowledge_publishing: 'git', project_system: 'github_issues', execution_federation: 'postgres'
  })
});
...
export function resolveBinding(capability, { project, env = process.env, mode } = {}) {
  ...
  if (bindingsJson?.modes?.[effectiveMode]?.[capability]) {
    const name = bindingsJson.modes[effectiveMode][capability];
    validateAdapter(capability, name);
    return name;
  }
