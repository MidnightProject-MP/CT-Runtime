...
export async function request(capability, { project, env = process.env, ...createOpts } = {}) {
  ...
  if (adapter.meta.remote || binding === 'gas') {
    return { capability: def.name, binding, provider: adapter.meta.provider, meta: adapter.meta, operations: [], authority: adapter.meta.authority, remote: true };
  }
  ...
  if (capability === 'disposable_compute' || capability === 'scheduler') {
    return { capability: def.name, binding, provider: adapter.meta.provider, meta: adapter.meta, operations: adapter.meta.operations, authority: adapter.meta.authority, ...adapter.create({ token: createOpts.token || env.CT_RUNTIME_NORTHFLANK_API_TOKEN, projectId: createOpts.projectId || env.CT_RUNTIME_NORTHFLANK_PROJECT_ID, ...createOpts }) };
  }
