...
const FREE_MODEL = /^openrouter\/[A-Za-z0-9][A-Za-z0-9._:/-]{0,247}:free$/;
...
// Rules: arrays map `undefined`/holes to `null` (JSON.stringify array semantics),
// object keys with `undefined` values are omitted, object key ordering is ordinal
// via `String` comparison (`Array#sort` without comparator, i.e. `Object.keys(...).sort()`).
// This preserves legacy hashes that relied on ordinal ordering, not locale-aware sort.
...
export function loadConfig(env = process.env) {
  ...
  const config = {
    ...
    providerSecretNames: providerSecretNames(env.CT_RUNTIME_PROVIDER_SECRET_NAMES || 'OPENROUTER_API_KEY'),
    freeOnly: env.CT_RUNTIME_FREE_ONLY === 'true',
...
export function validateModelPolicy(model, { mode = 'filesystem', freeOnly = false } = {}) {
  if (mode === 'production' && freeOnly && !FREE_MODEL.test(model)) throw new Error('production free-only mode requires a bounded OpenRouter :free model');
  return model;
}
