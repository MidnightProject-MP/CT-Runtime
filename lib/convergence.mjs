...
export function normalizeWorkUnit(input = {}) {
  ...
  return { ...value, intentDigest: sha256({ intendedOutcome: value.intendedOutcome, invariants: value.invariants, evidenceRequirements: value.evidenceRequirements }) };
}
...
export function normalizeCheckResult(input = {}, workUnit, subject) {
  ...
  return { ...check, checkId: sha256(check) };
}
...
export function reconcileConvergence({ workUnit, subject, checks = [] } = {}) {
  ...
  const result = missing.length || valid.some((check) => check.result === 'indeterminate')
    ? 'indeterminate'
    : valid.some((check) => check.result === 'fail') ? 'not ready'
      : required.size === 0 || [...required].every((name) => valid.find((check) => check.checkName === name)?.result === 'pass') ? 'ready' : 'indeterminate';
  return { schema: CONVERGENCE_SCHEMA, workUnitId: unit.workUnitId, pullRequestId: pr.pullRequestId, headSha: pr.headSha, result, missing, checkIds: valid.map((check) => check.checkId), canonicalCommit: null };
}
