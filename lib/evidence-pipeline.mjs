...
export async function exportOpenCodeSession({ sessionId, store, evidenceStore, project, command, run, extractionMode = 'sanitized', semanticEnvelope, resolveSemanticSource } = {}) {
  // resolveSemanticSource is required when attaching a new semantic envelope or when a reusable
  // envelope already exists for this session (hash verification needs durable bytes); it may be
  // omitted for sanitized-only evidence with no reusable envelope.
  ...
  if (evidenceStore) {
    ...
    try {
      ...
      console.warn(`opencode remote delivery failed for ${revisedEvidence.provenance.sourceSessionId}: ${message}`);
      delivery = { local, remote: { success: false, error: message } };
    }
  }
  if (isDuplicate) {
...
// Reuses a previously attached semantic envelope within the same session to prevent silent stripping.
// Lock TTL expectations: `store.lock('opencode-revision-publication')` defaults to ~300s; the
// critical section only performs bounded local reads/writes (no remote I/O), so it completes
// within milliseconds. Remote `evidenceStore.put` is intentionally performed *outside* the lock
// and may take seconds without risking TTL expiry. If conflicting envelopes are detected
// (distinct `contentHash` for the same `sourceSessionId`), this fails closed — operator must
// reconcile the canonical inbox manually (inspect `observer/inbox/opencode-session-*` files,
// retain the intended envelope, archive the conflicting one).
