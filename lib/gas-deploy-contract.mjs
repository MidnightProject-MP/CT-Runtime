...
export function deploymentIdFromWebAppUrl(value) {
  let url;
  try { url = new URL(String(value)); } catch (_) { throw new Error('invalid-deploy-url'); }
  if (url.protocol !== 'https:' || url.hostname !== 'script.google.com') throw new Error('invalid-deploy-url');
  const match = url.pathname.match(/^\/macros\/s\/([A-Za-z0-9_-]+)\/exec\/?$/);
  if (!match) throw new Error('invalid-deploy-url');
  return match[1];
}
...
export function signingString(request, timestamp, nonce, body) {
  ...
  return [request.operation, timestamp, nonce, request.deployment_request_id ?? '', request.script_id ?? '', request.deployment_id ?? '', request.commit_sha ?? '', request.github_bundle_hash ?? '', bodyHash].join('\n');
}
