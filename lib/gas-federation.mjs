...
export class PostgresGasAdvisoryAdapter {
  ...
  async persist(advisory) {
    ...
    try {
      ...
      const handoff = (await client.query(`SELECT h.*, s.checkpoint AS source_checkpoint
      ...
      if (!handoff || !target || handoff.to_execution_id !== value.targetExecutionId || String(target.claim_fence) !== String(value.targetFence) || target.claim_owner !== value.instanceId || sha256(handoff.source_checkpoint || null) !== value.checkpointDigest) throw new Error('advisory target or source checkpoint is stale');
      ...
      return { advisoryId: value.nonce, state, duplicate: !result.rowCount };
