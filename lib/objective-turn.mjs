...
const BASE_KEYS = ['objective_id', 'disposition', 'summary', 'provenance'];
...
function provenance(value) {
  ...
  const result = {};
  for (const key of ['provider', 'model', 'identity_ref', 'identity_revision', 'prompt_version', 'contract_version']) {
    result[key] = text(value[key], `provenance.${key}`, 256);
  }
...
export function validateObjectiveTurn(result) {
  ...
  if (result.provenance !== undefined) turn.provenance = provenance(result.provenance);
