...
export class PostgresObserverStore {
  ...
  async getJoin(executionId, client = this.pool) { const result = await client.query('SELECT join_record FROM observer_evidence_joins WHERE execution_id=$1', [safeId(executionId)]); return result.rows[0]?.join_record; }

  async appendJoin(join) {
    const { validateEvidenceJoin, createSemanticObservationTask } = await this.foundry();
    const result = await this.transaction(async (client) => {
      const digest = (await client.query('SELECT digest FROM observer_digests WHERE execution_id=$1 FOR UPDATE', [safeId(join.executionId)])).rows[0];
      if (!digest) throw new Error('Observer digest not found');
      const checked = validateEvidenceJoin(join, digest.digest);
      const inserted = await client.query(`INSERT INTO observer_evidence_joins(execution_id,binding_hash,claim_references,envelope,join_record,created_at) VALUES ($1,$2,$3,$4,$5,clock_timestamp()) ON CONFLICT (execution_id) DO NOTHING RETURNING execution_id`, [checked.executionId, checked.bindingHash, checked.claimReferences, checked.envelope, checked]);
      const current = inserted.rowCount ? checked : (await client.query('SELECT join_record FROM observer_evidence_joins WHERE execution_id=$1 FOR UPDATE', [checked.executionId])).rows[0].join_record;
      if (immutableJoinContent(current) !== immutableJoinContent(checked)) throw new Error('Conflicting immutable evidence join already exists');
      const task = createSemanticObservationTask(digest.digest, current);
      if (inserted.rowCount) {
        await appendArtifact(client, { executionId: checked.executionId, kind: 'semantic-task', key: `${checked.executionId}:${checked.bindingHash}`, content: task });
        const execution = (await client.query('SELECT observer_state FROM runtime_executions WHERE execution_id=$1 FOR UPDATE', [checked.executionId])).rows[0];
        await client.query("UPDATE observer_digests SET lifecycle_state='semantic-analysis-pending' WHERE execution_id=$1", [checked.executionId]);
        await client.query("UPDATE runtime_executions SET observer_state='pending',observer_pending_at=COALESCE(observer_pending_at,clock_timestamp()),version=version+1 WHERE execution_id=$1", [checked.executionId]);
        await appendLifecycle(client, checked.executionId, execution?.observer_state || 'not-eligible', 'semantic-analysis-pending', 'pending', { schema: 'celestan-observer-lifecycle-v1' });
      }
      return { status: inserted.rowCount ? 'processed' : 'duplicate', executionId: checked.executionId, task };
    });
    await this.mirrorArtifact('semantic-task', `${result.executionId}:${join.bindingHash}`, result.task);
    return result;
  }

  async markSemanticEvidenceInsufficient(executionId) {
    safeId(executionId);
    return this.transaction(async (client) => {
      const digest = (await client.query('SELECT lifecycle_state FROM observer_digests WHERE execution_id=$1 FOR UPDATE', [executionId])).rows[0];
      if (!digest) throw new Error('Observer digest not found');
      ...
      if (digest.lifecycle_state !== 'semantic-evidence-insufficient') await appendLifecycle(client, executionId, digest.lifecycle_state, 'semantic-evidence-insufficient', 'ineligible', { schema: 'celestan-observer-lifecycle-v1', reason: 'no-eligible-claim-envelope' });
      if (digest.lifecycle_state !== 'semantic-evidence-insufficient') await client.query("UPDATE observer_digests SET lifecycle_state='semantic-evidence-insufficient' WHERE execution_id=$1", [executionId]);
      if (execution?.observer_state !== 'not-eligible') await client.query("UPDATE runtime_executions SET observer_state='not-eligible',version=version+1 WHERE execution_id=$1", [executionId]);
      return { status: execution?.observer_state === 'not-eligible' && digest.lifecycle_state === 'semantic-evidence-insufficient' ? 'duplicate' : 'ineligible', executionId };
    });
  }
  ...
  async append(digest) {
    ...
    const result = await this.transaction(async (client) => {
      ...
      const join = await this.getJoin(executionId, client);
      const semantic = await this.getSemantic(executionId, client);
      const state = semantic ? 'observed' : join ? 'semantic-analysis-pending' : 'semantic-evidence-insufficient';
      ...
      await client.query(`UPDATE runtime_executions SET observer_state=$2,
        observer_pending_at=CASE WHEN $2='pending' THEN COALESCE(observer_pending_at,clock_timestamp()) ELSE observer_pending_at END,
         ...
         version=version+1 WHERE execution_id=$1`, [executionId, state === 'observed' ? 'observed' : state === 'semantic-evidence-insufficient' ? 'not-eligible' : 'pending']);
      if (inserted.rowCount && join && !semantic) await appendArtifact(client, { executionId, kind: 'semantic-task', key: executionId, content: createSemanticObservationTask(existing, join) });
      if (inserted.rowCount) await appendLifecycle(client, executionId, execution.observer_state || 'not-eligible', state, state === 'semantic-evidence-insufficient' ? 'ineligible' : 'pending', { schema: 'celestan-observer-lifecycle-v1' });
      return { status: inserted.rowCount ? 'processed' : 'duplicate', executionId, task: join ? createSemanticObservationTask(existing, join) : undefined, state };
    });
    if (result.task) await this.mirrorArtifact('semantic-task', executionId, result.task);
    return result;
  }
  ...
  async appendSemantic(executionId, output) {
    ...
    const validatedJoin = (await this.foundry()).validateEvidenceJoin(join, digest);
...
function immutableJoinContent(join) { return canonicalJson({ schema: join.schema, executionId: join.executionId, bindingHash: join.bindingHash, claimReferences: join.claimReferences, envelope: join.envelope }); }
