...
export class PostgresObserverStore {
  ...
  async append(digest) {
    ...
    const result = await this.transaction(async (client) => {
      ...
      if (!semantic) await appendArtifact(client, { executionId, kind: 'semantic-task', key: executionId, content: task });
...
async function appendArtifact(client, { executionId = null, kind, key, content, markdown = null }) {
  ...
  if (!inserted.rowCount) {
    ...
    const existingHash = hash({ content: existing.content, markdown: existing.markdown });
    if (existingHash !== contentHash) {
      console.error(`artifact conflict ${kind} ${key} existingHash ${existingHash} newHash ${contentHash}`, JSON.stringify({ existing: existing.content, next: content }).slice(0, 4000));
      throw new Error(`immutable Observer ${kind} artifact conflict`);
    }
  }
