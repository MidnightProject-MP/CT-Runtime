...
// Operational cost: fully reads and parses every `.json` file in `observer/inbox`.
// Typical inbox is small (tens to low hundreds of canonical evidence files) so
// a full scan is bounded. For larger deployments (>~1k files) this becomes
// noticeable; operators should schedule inbox maintenance (archive/process) and
// we should consider a flag/paging or index-assisted TODO before it grows unbounded.
// JSON parse failures for `opencode-session-*` include only the filename, not contents,
// to avoid echoing source payloads in diagnostics (non-OpenCode files are silently skipped).
...
// Matches identifier segments that look like credential material (token/secret/password, etc.)
// delimited by the allowed identifier separators. Any match is treated as sensitive and
// conservatively replaced with an opaque `opaque-<hash>` identifier rather than persisted.
...
// reusableEnvelope (in lib/evidence-pipeline.mjs) runs inside `store.lock('opencode-revision-publication')`
// which defaults to 300s; only bounded local reads/writes occur inside the lock, while remote
// `evidenceStore.put` is performed after releasing the lock and may take seconds without
// expiring the TTL. Conflicting envelopes (distinct `contentHash` for same session) throw a
// fail-closed actionable error: operator must manually reconcile the inbox and archive the
// unintended `opencode-session-*.json` file (future helper CLI TODO).
...
/**
 * Verify every source in a semantic envelope by re-hashing its durable bytes.
 *
 * @param {object} envelope - Validated semantic evidence envelope (from `validateSemanticEnvelope`).
 * @param {(source: object) => Promise<Buffer|Uint8Array|string>} resolveSemanticSource - Async resolver
 *   called once per `envelope.sources` entry with the source descriptor object
 *   (`{ sourceId, sourceClass, reference, sha256, ... }`). Must return raw bytes
 *   as `Buffer`, `Uint8Array`, or `string`. If the source cannot be found it may
 *   return `null`/`undefined` or throw — either case is surfaced as
 *   `semantic envelope source is unavailable: <sourceId>`. A hash mismatch
 *   against `source.sha256` is fatal and throws `semantic envelope source hash mismatch`.
 * @returns {Promise<object>} The same envelope on success.
 * @example
 *   // Minimal resolver used in tests — map a sourceId to durable bytes:
 *   const resolveSemanticSource = async (source) => {
 *     // return null/throw if not found; bytes are hashed and compared to source.sha256
 *     return sourceBytesById[source.sourceId]; // Buffer | Uint8Array | string
 *   };
 *   await verifySemanticEnvelopeSources(envelope, resolveSemanticSource);
 */
