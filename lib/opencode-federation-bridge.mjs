...
export function createOpenCodeFederationBridge(adapter, { secrets = [] } = {}) {
  ...
  return Object.freeze({
    ...
    async discover(input) { return adapter.discoverResumableWork(input); },
    async takeover(input) { return adapter.takeoverResumableWork({ ...input, repository: redactFederation(input.repository, secrets) }); },
    lineage(input) { return normalizeObserverLineage(input); }
  });
}
export const OPEN_CODE_SEMANTIC_BOUNDARIES = Object.freeze(['begin', 'checkpoint', 'defer', 'continue', 'finalize', 'discover', 'takeover', 'lineage']);
