...
const observerTaskType = (purpose) => ['implementation', 'debugging', 'review', 'verification', 'research', 'planning', 'architecture', 'documentation', 'operations', 'orchestration'].find((type) => String(purpose).toLowerCase().includes(type)) || 'other';
const observerTelemetry = (records) => ({ availability: 'available', invocations: records.map((record) => ({ invocationId: record.recordId, provider: record.route.provider, model: record.route.model, agent: record.identity.agent, role: record.identity.role, task: { type: observerTaskType(record.task.purpose), label: record.task.purpose }, startedAt: record.timing.startedAt, finishedAt: record.timing.finishedAt, durationMs: record.timing.latencyMs, retry: { attempt: record.retry.attempt, retryCount: record.retry.retries }, status: record.outcome, outcome: record.outcome })), failures: records.filter((record) => record.failure).map((record) => ({ failureId: `failure-${record.recordId}`, invocationId: record.recordId, category: record.failure.foundryCategory, errorClass: record.failure.class, timestamp: record.timing.finishedAt, retryable: record.failure.retryable, retryCount: record.retry.retries })), transitions: [] });
...
export class PostgresStore {
  constructor({ connectionString, config, pool, evidenceStore } = {}) {
    ...
    this.federation = new PostgresFederationAdapter({ pool: this.pool });
  }
  ...
  async manifestsAll() {
    ...
    return Promise.all(result.rows.map(async (value) => {
      const manifest = fromExecution(value);
      const records = await this.inferenceRecords({ executionId: manifest.execution.id });
      return records.length ? { ...manifest, modelRuntimeTelemetry: observerTelemetry(records) } : manifest;
    }));
  }
  ...
  async inferenceRecord(record) {
    if (record?.schema !== 'celestan-runtime-inference-v1') throw new Error('normalized inference record is required');
    await this.telemetry({ executionId: record.logical.executionId, attempt: record.retry.attempt, kind: 'normalized-inference', record });
    return record;
  }

  async inferenceRecords({ executionId } = {}) {
    if (!executionId) throw new Error('Postgres inference record reads require executionId');
    return (await this.modelTelemetryFor(executionId)).filter((item) => item.kind === 'normalized-inference').map((item) => item.record);
  }
  ...
  async persistSemanticEvidence(executionId, envelope, lease) {
    const checked = validateSemanticEvidence(envelope);
    if (checked.lineage.physicalExecutionId !== executionId) throw conflictError('semantic evidence execution');
    const data = Buffer.from(JSON.stringify(checked), 'utf8');
    return this.transaction(async (client) => {
      await lockLease(client, executionId, lease);
      const execution = row(await client.query('SELECT execution_id,work_order FROM runtime_executions WHERE execution_id=$1 FOR UPDATE', [executionId]));
      if (!execution) throw new Error('execution not found');
      if (execution.work_order && checked.lineage.workOrderId !== execution.work_order) throw conflictError('semantic evidence work order');
      const inserted = row(await client.query(`
        INSERT INTO runtime_semantic_evidence(envelope_id,execution_id,content_hash,envelope,byte_count,created_at)
        VALUES ($1,$2,$3,$4,$5,clock_timestamp())
        ON CONFLICT (envelope_id) DO NOTHING RETURNING *`,
      [checked.envelopeId, executionId, checked.contentHash, checked, data.length]));
      const current = inserted || row(await client.query('SELECT * FROM runtime_semantic_evidence WHERE envelope_id=$1 FOR SHARE', [checked.envelopeId]));
      if (current.execution_id !== executionId) throw conflictError('semantic evidence immutable identity');
      if (current.content_hash !== checked.contentHash || !equal(current.envelope, checked) || !bigintEqual(current.byte_count, data.length)) throw conflictError('semantic evidence immutable content');
      return { envelopeId: checked.envelopeId, executionId, sha256: checked.contentHash, bytes: data.length };
    });
  }

  async semanticEvidenceFor(executionId, options = {}) {
    const limit = typeof options === 'number' ? options : options.limit ?? 100;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 100) throw new Error('semantic evidence limit must be from 1 to 100');
    const result = await this.pool.query('SELECT envelope FROM runtime_semantic_evidence WHERE execution_id=$1 ORDER BY created_at,envelope_id LIMIT $2', [executionId, limit]);
    const manifest = await this.manifest(executionId);
    return result.rows.map((item) => {
      const checked = validateSemanticEvidence(item.envelope);
      if (checked.lineage.physicalExecutionId !== executionId || manifest.workOrder && checked.lineage.workOrderId !== manifest.workOrder) throw new Error('semantic evidence linkage is invalid');
      return checked;
    });
  }
