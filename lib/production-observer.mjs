...
export async function observePostgres({ store, observerStore, semanticResultFile, reflection } = {}) {
  ...
  for (const manifest of await store.manifestsAll()) {
    ...
    const input = { execution, evidence: { source: 'ct-runtime', references, evidenceDigest: sha(canonicalJson(evidenceRows)) } };
    let appended;
    try {
      appended = await foundry.digestAndAppend(observerStore, input);
      ...
      const semantic = semanticResultFile
        ...
        : reflection ? await invokeReflection(appended.result.task || foundry.createSemanticObservationTask(appended.digest), reflection) : undefined;
      ...
      console.error(`observePostgres failure ${executionId}:`, error.stack || error.message, error.cause ? String(error.cause) : '');
      if (appended) await observerStore.appendSemanticFailure(executionId, safeFailure(error), 1).catch(() => {});
      const f = safeFailure(error); f.message = String(error.message).slice(0, 500); output.push({ executionId, status: 'semantic-analysis-pending', failure: f });
    }
  }
  return output;
}
