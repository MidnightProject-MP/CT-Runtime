import { canonicalFeedbackEvaluationContent, evaluationConflict, feedbackEvaluationId } from './feedback-evaluation.mjs';
...
export class PostgresFeedbackEvaluationStore {
  ...
  async persistEvaluation(evaluation) {
    const expectedId = feedbackEvaluationId(evaluation.feedback_id, evaluation.source_revision);
    if (evaluation.evaluation_id !== expectedId) throw evaluationConflict('feedback evaluation identity is invalid', evaluation);
    const expectedReceiptEventId = `human-feedback:${evaluation.feedback_id}:${evaluation.source_revision}`;
    if (evaluation.receipt_event_id !== expectedReceiptEventId) throw evaluationConflict('feedback receipt provenance is invalid', evaluation);
    ...
    try {
      ...
      const actual = {
        evaluation_id: current.evaluation_id, feedback_id: current.feedback_id, source_revision: Number(current.source_revision),
