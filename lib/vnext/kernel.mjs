...
export function applyTurn(workUnit, execution, turn, { authorizeTerminal = () => false, finishedAt } = {}) {
  ...
  const valid = validateObjectiveTurn({ ...turn, objective_id: workUnit.objective_ref });
...
export function settleReview(workUnit, { authorized }) {
  if (workUnit.state !== 'review') throw new Error('Work Unit is not awaiting outcome review');
  return { ...workUnit, state: authorized ? 'terminal' : 'actionable' };
}
