...
export function createMemoryStore({ workUnits = [], executions = [], authorities = null, authorizationVerifier = null } = {}) {
  ...
  for (const workUnit of work.values()) {
    ...
    if (execution.work_unit_id !== workUnit.work_unit_id || execution.project_id !== workUnit.project_id || !execution.authorization_decision_ref || execution.fence !== claim.fence || execution.owner !== claim.owner || !['created', 'running'].includes(execution.state)) {
