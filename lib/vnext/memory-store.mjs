...
export function createMemoryStore({ workUnits = [], executions = [], authorities = null } = {}) {
  ...
  return {
    ...
    async beginExecution(workUnit, execution) {
      ...
      const now = Date.now();
      if (current.claim && (current.claim.claim_expires_at !== execution.claim_expires_at || current.claim_expires_at !== execution.claim_expires_at)) throw new Error('execution claim expiration does not match Work Unit claim');
