...
export function createMemoryStore({ workUnits = [], executions = [], authorities = null, authorizationVerifier = null } = {}) {
  ...
  for (const workUnit of work.values()) {
    ...
    if (execution.work_unit_id !== workUnit.work_unit_id || execution.project_id !== workUnit.project_id || (!execution.authorization_decision_ref && !legacyHistoricalExecution) || execution.fence !== claim.fence || execution.owner !== claim.owner || !['created', 'running', 'expired'].includes(execution.state)) {
  ...
  return {
    ...
    async beginExecution(workUnit, execution, authorizationDecision = null) {
      ...
      state.authorities.set(execution.project_id, { project_id: execution.project_id, work_unit_id: workUnit.work_unit_id, execution_id: execution.execution_id, fence: execution.fence, owner: execution.owner, claim_expires_at: execution.claim_expires_at, authorization_decision_ref: execution.authorization_decision_ref });
