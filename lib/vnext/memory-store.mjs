...
export function createMemoryStore({ workUnits = [], executions = [] } = {}) {
  ...
  return {
    ...
    async beginExecution(workUnit, execution) {
      ...
      if (authority && !expiredAuthority) throw new Error('project mutation authority is already held');
      ...
      state.authorities.set(execution.project_id, { project_id: execution.project_id, work_unit_id: workUnit.work_unit_id, execution_id: execution.execution_id, fence: execution.fence, claim_expires_at: execution.claim_expires_at });
