...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function persistTurn(result) {
    ...
    try {
      ...
      const exec = await client.query('SELECT * FROM vnext_executions WHERE execution_id=$1 AND work_unit_id=$2 AND owner=$3 AND fence=$4 FOR UPDATE', [execution.execution_id, workUnit.work_unit_id, execution.owner, execution.fence]);
      if (!exec.rows[0] || exec.rows[0].state !== 'running') throw new Error('execution is not running');
      await client.query('UPDATE vnext_executions SET state=$2,finished_at=$3,failure=$4::jsonb,retry_after=$5 WHERE execution_id=$1 AND work_unit_id=$6 AND owner=$7 AND fence=$8', [execution.execution_id, execution.state, execution.finished_at, json(execution.failure), execution.retry_after, workUnit.work_unit_id, execution.owner, execution.fence]);
  ...
  async function persistFailure(failed) {
    ...
    try {
      ...
      const updated = await client.query("UPDATE vnext_executions SET state=$2,finished_at=$3,failure=$6::jsonb,retry_after=$7 WHERE execution_id=$1 AND work_unit_id=$8 AND owner=$4 AND fence=$5 AND state IN ('created','running')", [execution.execution_id, execution.state, execution.finished_at, execution.owner, execution.fence, json(execution.failure), execution.retry_after, workUnit.work_unit_id]);
