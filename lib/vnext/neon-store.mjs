...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function beginExecution(workUnit, execution) {
    ...
    try {
      ...
      if (!['actionable', 'waiting'].includes(row.state) || row.claim_execution_id || Number(row.fence) !== expectedPreviousFence) throw new Error('Work Unit changed before execution could be claimed');
  ...
  async function persistFailure(failed) {
    ...
    try {
      ...
      const updated = await client.query("UPDATE vnext_executions SET state=$2,finished_at=$3 WHERE execution_id=$1 AND owner=$4 AND fence=$5 AND state IN ('created','running')", [execution.execution_id, execution.state, execution.finished_at, execution.owner, execution.fence]);
