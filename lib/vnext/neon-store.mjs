...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function beginExecution(workUnit, execution) {
    ...
    try {
      ...
      if (row.claim_execution_id && (!sameTime(row.claim_expires_at, execution.claim_expires_at) || !sameTime(row.claim_expires_at, workUnit.claim_expires_at))) throw new Error('execution claim expiration does not match Work Unit claim');
