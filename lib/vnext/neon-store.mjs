...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function beginExecution(workUnit, execution) {
    ...
    try {
      ...
      const authorityInserted = await client.query('INSERT INTO vnext_project_mutation_authority(project_id,work_unit_id,execution_id,fence,claim_expires_at) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING RETURNING *', [execution.project_id, workUnit.work_unit_id, execution.execution_id, execution.fence, execution.claim_expires_at]);
      if (authorityInserted.rowCount !== 1) throw new Error('project mutation authority is already held');
