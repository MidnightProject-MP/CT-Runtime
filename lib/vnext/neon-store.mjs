...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function beginExecution(workUnit, execution) {
    ...
    try {
      ...
      await client.query('INSERT INTO vnext_project_mutation_authority(project_id,work_unit_id,execution_id,fence,claim_expires_at) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING', [execution.project_id, workUnit.work_unit_id, execution.execution_id, execution.fence, execution.claim_expires_at]);
      ...
      if (!authority.rows[0]) throw new Error('project mutation authority could not be acquired');
      const authorityRow = authority.rows[0];
      const authorityExpired = new Date(authorityRow.claim_expires_at).getTime() <= Date.now();
      if (authorityRow.execution_id !== execution.execution_id) {
        if (!authorityExpired) throw new Error('project mutation authority is already held');
        ...
        await client.query('INSERT INTO vnext_project_mutation_authority(project_id,work_unit_id,execution_id,fence,claim_expires_at) VALUES ($1,$2,$3,$4,$5)', [execution.project_id, workUnit.work_unit_id, execution.execution_id, execution.fence, execution.claim_expires_at]);
      }
  ...
  async function persistTurn(result) {
    ...
    try {
      ...
      const authority = await client.query('SELECT * FROM vnext_project_mutation_authority WHERE project_id=$1 FOR UPDATE', [execution.project_id]);
      if (!authority.rows[0] || authority.rows[0].work_unit_id !== workUnit.work_unit_id || authority.rows[0].execution_id !== execution.execution_id || Number(authority.rows[0].fence) !== execution.fence) throw new Error('fencing conflict while persisting turn');
  ...
  async function persistFailure(failed) {
    ...
    try {
      ...
      const authority = await client.query('SELECT * FROM vnext_project_mutation_authority WHERE project_id=$1 FOR UPDATE', [execution.project_id]);
      if (!authority.rows[0] || authority.rows[0].work_unit_id !== workUnit.work_unit_id || authority.rows[0].execution_id !== execution.execution_id || Number(authority.rows[0].fence) !== execution.fence) throw new Error('fencing conflict while persisting failure');
      const updated = await client.query("UPDATE vnext_executions SET state=$2,finished_at=$3,failure=$6::jsonb,retry_after=$7 WHERE execution_id=$1 AND work_unit_id=$8 AND project_id=$9 AND owner=$4 AND fence=$5 AND state IN ('created','running')", [execution.execution_id, execution.state, execution.finished_at, execution.owner, execution.fence, json(execution.failure), execution.retry_after, workUnit.work_unit_id, workUnit.project_id]);
