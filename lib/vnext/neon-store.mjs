...
export function createNeonStore({ connectionString, pool, applicationName = 'ct-runtime-vnext' } = {}) {
  ...
  async function appendEvent(event) {
    ...
    const result = await db.query('INSERT INTO vnext_events(event_id,event_type,payload) VALUES ($1,$2,$3::jsonb) ON CONFLICT (event_id) DO NOTHING RETURNING event_id', [event.event_id, event.type, json(event)]);
