...
import { createObjectiveExecutor } from './objective-host.mjs';
...
export async function runOuterLoop({ wake, store, executor, host, isJustified = ({ workUnit }) => workUnit.state === 'actionable', owner = 'vnext', authorizeTerminal, maxAttempts = 3, retryDelayMs = 1000 } = {}) {
  ...
  if (executor !== undefined && host !== undefined) throw new Error('executor and host are mutually exclusive');
  if (executor !== undefined && typeof executor !== 'function') throw new Error('executor must be a function');
  if (!executor && host) executor = createObjectiveExecutor(host);
  if (typeof executor !== 'function') throw new Error('executor or host is required');
