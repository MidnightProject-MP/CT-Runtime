-- A pending advisory may outlive a foreground lease, but it must never revive
-- an execution superseded by a newer work-order fence or takeover handoff.
...
BEGIN
  ...
  IF w.work_order_id IS NULL OR h.handoff_id IS NULL OR e.execution_id IS NULL OR h.to_execution_id<>e.execution_id OR e.claim_fence<>a.target_fence OR w.next_claim_fence<>a.target_fence OR e.state NOT IN ('claimed','running') THEN
    UPDATE public.federation_advisories SET state='rejected',consumed_at=pg_catalog.clock_timestamp() WHERE advisory_id=a.advisory_id;
