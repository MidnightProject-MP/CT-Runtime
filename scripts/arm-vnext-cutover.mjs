...
const operation = 'arm-vnext-cutover';
...
const response = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body, redirect: 'follow' });
const text = await response.text();
let result;
try { result = JSON.parse(text); } catch (_) { throw new Error(`invalid GAS response: HTTP ${response.status}`); }
console.log(JSON.stringify({ operation, diagnostic_id: request.correlation_id, body_sha256: createHash('sha256').update(body, 'utf8').digest('hex'), canonical_sha256: createHash('sha256').update(canonical, 'utf8').digest('hex'), http_status: response.status, response_body: text }));
if (!response.ok) throw new Error(`GAS control-plane HTTP ${response.status}: ${JSON.stringify(result)}`);
if (result.status !== 'VNEXT_ARMED' || result.autonomy_mode !== 'vnext') throw new Error(`unexpected arm result: ${JSON.stringify(result)}`);
console.log(JSON.stringify({ operation, ...result }));
if (process.env.OUTPUT_PATH) {
  const { writeFile } = await import('node:fs/promises');
  await writeFile(process.env.OUTPUT_PATH, JSON.stringify({ operation, ...result }, null, 2) + '\n', 'utf8');
}
